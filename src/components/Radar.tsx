...
import { destinations, destKey, hoverKey, origin, results, setHoverKey } from "../store";
    ...
    >
      ...
      <For each={rings()}>
        {(min) => {
          ...
          return (
            <>
              <circle cx={C} cy={C} r={r()} fill="none" stroke="white" stroke-opacity="0.07" />
              <text
                x={C}
                y={C - r() - 5}
                text-anchor="middle"
      ...
      <circle
        ...
        stroke-width="2.5"
      />
      <text x={C} y={C + 24} text-anchor="middle" class="fill-ink" font-size="13" font-weight="700">
        {origin()?.name}
      </text>
        ...
        {(p) => (
          ...
          >
            <Show when={hoverKey() === p.key}>
              <circle r={13} fill="none" stroke={p.color} stroke-opacity="0.6" stroke-width="2" />
            </Show>

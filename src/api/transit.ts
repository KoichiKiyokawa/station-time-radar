...
export async function suggestStations(
  ...
  const res = await fetch(
    `${BASE}/api/v1/locations/suggest?q=${encodeURIComponent(query)}`,
    { signal },
  )
  ...
  for (const s of data.stations ?? []) {
    ...
    const g = clusters.find(
      (c) =>
        approxDistanceKm(c.station.lat, c.station.lon, s.lat, s.lon) <=
        SAME_STATION_KM,
    )
    if (!g) {
      clusters.push({
  ...
  return [...groupsByName.values()]
    ...
    .sort(
      (a, b) =>
        Number(b.station.isStation) - Number(a.station.isStation) ||
        b.weight - a.weight,
    )
    .slice(0, 8)
...
export async function planRoute(
  ...
  const candidates = (data.journeys ?? []).filter((j) =>
